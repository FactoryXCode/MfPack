...
type
  ...
  public
    ...
    function CheckCaptureDeviceLost(var pHdr: DEV_BROADCAST_HDR; var pbDeviceLost: BOOL): HRESULT;
...
//-------------------------------------------------------------------
function TMfCaptureEngine.CheckCaptureDeviceLost(var pHdr: DEV_BROADCAST_HDR;
                                                 var pbDeviceLost: BOOL): HRESULT;
var
  hr: HRESULT;
...
begin
  ...
  if succeeded(hr) then
    begin
      ...
      if Assigned(m_pwszSymbolicLink) and Assigned(pDi) then
        if (StrComp(m_pwszSymbolicLink,
                    PWideChar(@pDi^.dbcc_name)) = 0) then
          pbDeviceLost := True;
      pHdr.dbch_devicetype := pDi.dbcc_devicetype;
    end;
  Result := hr;
end;
