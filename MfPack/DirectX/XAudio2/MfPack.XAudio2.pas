...
{$ENDIF}   // Win 10
  ...
  **************************************************************************)
  ...
  IXAudio2Voice = class(TObject)
    ...
    procedure GetVolume(out pVolume: FLOAT); virtual; stdcall; abstract;
    ...
    procedure GetChannelVolumes(Channels: UINT32;
                                {out} pVolumes: PFloat); virtual; stdcall; abstract;
    ...
    procedure GetOutputMatrix(pDestinationVoice: IXAudio2Voice;
                              SourceChannels: UINT32;
                              DestinationChannels: UINT32;
                              {out} pLevelMatrix: PFloat); virtual; stdcall; abstract;
  ...
  **************************************************************************)
  ...
  IXAudio2SourceVoice = class(IXAudio2Voice)
    ...
    //  OperationSet - Used to identify this call as part of a deferred batch.
    //
    function SetFrequencyRatio(Ratio: FLOAT;
                               OperationSet: UINT32 = XAUDIO2_COMMIT_NOW): HRESULT; virtual; stdcall; abstract;
    ...
    //
    procedure GetFrequencyRatio(out pRatio: FLOAT); virtual; stdcall; abstract;
  ...
  **************************************************************************)
  ...
  function XAudio2DecibelsToAmplitudeRatio(Decibels: FLOAT): FLOAT; inline;
  {$EXTERNALSYM XAudio2DecibelsToAmplitudeRatio}

  function XAudio2AmplitudeRatioToDecibels(Volume: FLOAT): FLOAT; inline;
  {$EXTERNALSYM XAudio2AmplitudeRatioToDecibels}

  function XAudio2SemitonesToFrequencyRatio(Semitones: FLOAT): FLOAT; inline;
  {$EXTERNALSYM XAudio2SemitonesToFrequencyRatio}

  function XAudio2FrequencyRatioToSemitones(FrequencyRatio: FLOAT): FLOAT; inline;
  {$EXTERNALSYM XAudio2FrequencyRatioToSemitones}

  function XAudio2CutoffFrequencyToRadians(CutoffFrequency: FLOAT;
                                           SampleRate: UINT32): FLOAT; inline;
  {$EXTERNALSYM XAudio2CutoffFrequencyToRadians}

  function XAudio2RadiansToCutoffFrequency(Radians: FLOAT;
                                           SampleRate: FLOAT): FLOAT; inline;
  {$EXTERNALSYM XAudio2RadiansToCutoffFrequency}

  function XAudio2CutoffFrequencyToOnePoleCoefficient(CutoffFrequency: FLOAT;
                                                      SampleRate: UINT32): FLOAT; inline;
...
const
  ...
  M_PI : FLOAT = 3.14159265358979323846;   // From math.h
  {$EXTERNALSYM M_PI}
...
begin
  if (Volume = 0) then
    ...
    Result:= 20.0 * FLOAT(log10(Volume));
end;
...
begin
  if Round(CutoffFrequency * 6.0) >= SampleRate then
    ...
    Result:=  2.0 * Sin(FLOAT(M_PI) * CutoffFrequency / SampleRate);
end;
...
begin
    Result:= SampleRate * ArcSin(Radians / 2.0) / FLOAT(M_PI);
end;
