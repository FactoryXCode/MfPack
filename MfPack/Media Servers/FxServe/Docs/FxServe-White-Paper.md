...
The INI contains ordinary server and public-listener configuration only.  
It does not contain ACME account information, certificate thumbprints, private-key  
...
FxServeAdmin requires its application password at startup, following a  
administrative protection model. Its saved connection profile is  
...
FxServe does not remove a 3th party like Caddy support. A station can keep such installed and  
choose either server.  
  
Only one server can own ports 80 and 443 at a time. FxServe Admin manages only  
FxServe, while other serversn manages their additional settings.  
When changing servers, stop the active server with its own administration tool  
...
- [FxServe deployment guide](Deploy/PCHP001/README-Deploy.md)
