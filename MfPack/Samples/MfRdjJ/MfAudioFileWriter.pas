...
type
  ...
  TMfAudioFileWriter = class(TInterfacedObject, IAudioWriter)
  private
...
class constructor TMfAudioFileWriter.ClassCreate;
begin
...
class destructor TMfAudioFileWriter.ClassDestroy();
begin
...
constructor TMfAudioFileWriter.Create(AKind: TAudioOutputKind);
begin
...
destructor TMfAudioFileWriter.Destroy();
begin
...
function TMfAudioFileWriter.Kind(): TAudioOutputKind;
begin
...
function TMfAudioFileWriter.IsExtensible(const Wf: WAVEFORMATEX): Boolean;
begin
...
function TMfAudioFileWriter.GetExtensibleSubFormatGUID(const Wf: WAVEFORMATEX;
                                                       out SubFmt: TGUID): Boolean;
var
...
function TMfAudioFileWriter.IsFloatFormat(const Wf: WAVEFORMATEX): Boolean;
var
  sub: TGUID;
...
function TMfAudioFileWriter.GetChannelMask(const Wf: WAVEFORMATEX): UINT32;
var
  pExt: PWAVEFORMATEXTENSIBLE;
...
function TMfAudioFileWriter.MakeAudioType(const Major,
                                          SubType: TGUID;
                                          SampleRate,
                                          Channels,
                                          BitsPerSample,
...
function TMfAudioFileWriter.SetupTypes(const Wf: WAVEFORMATEX): HResult;
var
  hr: HResult;
...
function TMfAudioFileWriter.Open(const FileName: string;
                             const Wf: WAVEFORMATEX): HResult;
var
  hr: HResult;
...
procedure TMfAudioFileWriter.ConvertFloatToPCM24Packed(const Src: Pointer;
                                                   NumFrames: UINT32;
                                                   Channels: UINT32;
                                                   out DstPtr: Pointer;
                                                   out DstBytes: UINT32);
...
function TMfAudioFileWriter.GetValidBitsPerSample(const Wf: WAVEFORMATEX): UINT32;
var
  pExt: PWAVEFORMATEXTENSIBLE;
...
procedure TMfAudioFileWriter.ConvertPCM32ToPCM24Packed(const Src: Pointer;
                                                       NumFrames: UINT32;
                                                       Channels: UINT32;
                                                       out DstPtr: Pointer;
                                                       out DstBytes: UINT32);
...
function TMfAudioFileWriter.WriteFrames(const Data: Pointer;
                                        NumFrames: UINT32;
                                        const Wf: WAVEFORMATEX;
                                        var Time100ns: Int64;
                                        out BytesWritten: UINT32): HResult;
...
begin
        ...
        begin
          ...
          if (Data = nil) then
            ...
            begin
              ...
              if FSrcIsFloat then
                ConvertFloatToPCM24Packed(Data,
                                          NumFrames,
                                          FInChannels,
                                          srcPtr,
                                          bytesToWrite)
             else
               if (FSrcBitsPerSample = 32) then
                 ConvertPCM32ToPCM24Packed(Data,
                                           NumFrames,
                                           FInChannels,
                                           srcPtr,
                                           bytesToWrite)
  ...
  hr := MFCreateMemoryBuffer(bytesToWrite,
                             buf);
  if FAILED(hr) then
    Exit(hr);
  ...
  dur100ns := _MulDiv64(Int64(NumFrames),
                        10000000,
                        Int64(FInSampleRate));
  ...
  hr := FSinkWriter.WriteSample(FAudioStreamIndex,
                                sample);
  if FAILED(hr) then
    Exit(hr);
...
function TMfAudioFileWriter.Close(): HResult;
var
  hr: HResult;
...
begin
  ...
  if (ext = '.flac') then
    Result := TMfAudioFileWriter.Create(aokFlac)
  else  // .wav
    Result := TMfAudioFileWriter.Create(aokWav);
end;
