  ...
  case ABoxType of
    BOX_FTYP:
      begin
        ...
        OutputDebugString(PChar(
          Format('MfWebCamStreamer: ftyp bytes=%d total init=%d',
                 [Length(ABoxData),
                  Length(FInitSegment)])));
      end;
    ...
    BOX_MOOV:
      begin
        ...
        OutputDebugString(PChar(
          Format('MfWebCamStreamer: moov bytes=%d total init=%d',
                 [Length(ABoxData),
                  Length(FInitSegment)])));
      end;
...
begin
  ...
  try
    // ftyp alone is not a valid MSE initialization segment. The publisher
    // timer can run between the separate Media Foundation writes, so expose
    // init.mp4 only after both required top-level boxes have arrived.
    Result := FHaveFtyp and FHaveMoov and (Length(FInitSegment) > 0);
