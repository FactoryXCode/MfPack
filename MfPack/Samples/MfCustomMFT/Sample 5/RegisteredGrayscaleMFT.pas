...
const
  CLSID_FactoryXGrayscaleMFT: TGUID =
    '{8A761F4D-3E0F-4E37-A291-B6C11D5088A0}';
  FACTORYX_GRAYSCALE_MFT_NAME = 'FactoryX RGB32 Grayscale MFT';

type
  TMfGrayscaleMFT = class(TComObject, IMFTransform)
  private
    ...
    function GetVideoLayout(const AMediaType: IMFMediaType;
      out AWidth, AHeight: UINT32; out AStride: Integer): HResult;
    function TypesMatch(const AFirst, ASecond: IMFMediaType): Boolean;
  ...
  public
    ...
    function SetInputType(const dwInputStreamID: DWORD;
      pType: IMFMediaType; dwFlags: DWORD): HResult; stdcall;
...
function TMfGrayscaleMFT.CreateRgb32Type(
  out AMediaType: IMFMediaType): HResult;
begin
  AMediaType := nil;
  Result := MFCreateMediaType(AMediaType);
  if SUCCEEDED(Result) then
    Result := AMediaType.SetGUID(MF_MT_MAJOR_TYPE, MFMediaType_Video);
  if SUCCEEDED(Result) then
    Result := AMediaType.SetGUID(MF_MT_SUBTYPE, MFVideoFormat_RGB32);
end;
...
begin
  ...
  if not Assigned(AMediaType) then
    Exit(E_POINTER);
  if FAILED(AMediaType.GetGUID(MF_MT_MAJOR_TYPE, MajorType)) or
     not IsEqualGUID(MajorType, MFMediaType_Video) or
     FAILED(AMediaType.GetGUID(MF_MT_SUBTYPE, Subtype)) or
     not IsEqualGUID(Subtype, MFVideoFormat_RGB32) then
    Exit(MF_E_INVALIDMEDIATYPE);

  Result := MFGetAttributeSize(AMediaType, MF_MT_FRAME_SIZE,
                               AWidth, AHeight);
  ...
  if SUCCEEDED(AMediaType.GetUINT32(MF_MT_DEFAULT_STRIDE, StrideValue)) then
    begin
...
function TMfGrayscaleMFT.TypesMatch(const AFirst,
  ASecond: IMFMediaType): Boolean;
var
  Width1: UINT32;
  Height1: UINT32;
  Width2: UINT32;
  ...
  Stride2: Integer;
begin
  Result := SUCCEEDED(GetVideoLayout(AFirst, Width1, Height1, Stride1)) and
            SUCCEEDED(GetVideoLayout(ASecond, Width2, Height2, Stride2)) and
            (Width1 = Width2) and (Height1 = Height2) and
            (Stride1 = Stride2);
end;

function TMfGrayscaleMFT.ConvertSampleToGrayscale(
  const ASample: IMFSample): HResult;
var
...
function TMfGrayscaleMFT.GetStreamLimits(out pdwInputMinimum,
...
function TMfGrayscaleMFT.GetAttributes(
...
function TMfGrayscaleMFT.GetInputStreamAttributes(
  const dwInputStreamID: DWORD; out pAttributes: IMFAttributes): HResult;
begin
  pAttributes := nil;
  if dwInputStreamID <> 0 then
    Exit(MF_E_INVALIDSTREAMNUMBER);
...
function TMfGrayscaleMFT.GetOutputStreamAttributes(
  const dwOutputStreamID: DWORD; out pAttributes: IMFAttributes): HResult;
begin
  pAttributes := nil;
  if dwOutputStreamID <> 0 then
    Exit(MF_E_INVALIDSTREAMNUMBER);
...
begin
  ...
  if Assigned(FOutputType) then
    Result := CloneMediaType(FOutputType, pType)
  else
...
begin
  ...
  Result := GetVideoLayout(pType, Width, Height, Stride);
  if FAILED(Result) then
    Exit;
  if Assigned(FOutputType) and not TypesMatch(pType, FOutputType) then
    Exit(MF_E_INVALIDMEDIATYPE);
  if (dwFlags and MFT_SET_TYPE_TEST_ONLY) <> 0 then
    Exit(S_OK);

  Result := CloneMediaType(pType, NewType);
  if FAILED(Result) then
    Exit;
  FInputType := NewType;
  FWidth := Width;
...
function TMfGrayscaleMFT.ProcessOutput(dwFlags, cOutputBufferCount: DWORD;
  pOutputSamples: PMFT_OUTPUT_DATA_BUFFER;
...
initialization
  TComObjectFactory.Create(ComServer, TMfGrayscaleMFT,
    CLSID_FactoryXGrayscaleMFT, 'FactoryXGrayscaleMFT',
    FACTORYX_GRAYSCALE_MFT_NAME, ciMultiInstance, tmBoth);
