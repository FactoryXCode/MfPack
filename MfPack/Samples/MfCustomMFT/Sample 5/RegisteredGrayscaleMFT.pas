...
const
  CLSID_FactoryXGrayscaleMFT: TGUID = '{8A761F4D-3E0F-4E37-A291-B6C11D5088A0}';
...
begin
  ...
  try
    ...
    if (UInt64(CurrentLength) < RequiredLength) or (UInt64(MaxLength) < RequiredLength) then
...
function TMfGrayscaleMFT.GetStreamCount(out pcInputStreams: DWORD;
...
begin
  ...
  if Assigned(FInputType) and not TypesMatch(FInputType,
                                             pType) then
    Exit(MF_E_INVALIDMEDIATYPE);
...
function TMfGrayscaleMFT.SetOutputBounds(hnsLowerBound: LONGLONG;
...
function TMfGrayscaleMFT.ProcessOutput(dwFlags: DWORD;
...
initialization
  ...
  TComObjectFactory.Create(ComServer,
                           TMfGrayscaleMFT,
                           CLSID_FactoryXGrayscaleMFT,
                           'FactoryXGrayscaleMFT',
