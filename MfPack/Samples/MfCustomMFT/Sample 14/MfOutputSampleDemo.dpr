...
begin
  ...
  Check('GetOutputStreamInfo',
        Transform.GetOutputStreamInfo(0,
                                      StreamInfo));
  if ((StreamInfo.dwFlags and MFT_OUTPUT_STREAM_CAN_PROVIDE_SAMPLES) = 0) or
     ((StreamInfo.dwFlags and MFT_OUTPUT_STREAM_PROVIDES_SAMPLES) <> 0) or
     (StreamInfo.cbSize <> 1) then
    raise Exception.Create('Wrong output allocation contract');
  ...
  // Supply a sample with too little storage. ProcessOutput must retain the
  // pending input and the delay history so the caller can retry.
  Check('Flush before caller output',
        Transform.ProcessMessage(MFT_MESSAGE_COMMAND_FLUSH,
                                 0));
  Input := MakeSample([129,
                       130]);
  Check('Set caller input time',
        Input.SetSampleTime(0));
  Check('Set caller input duration',
        Input.SetSampleDuration(20000));
  Check('Set caller input flags',
        Input.SetSampleFlags(1));
  Check('Set caller input attribute',
        Input.SetUINT32(MFSampleExtension_CleanPoint,
                        1));
  Check('Caller input',
        Transform.ProcessInput(0,
                               Input,
  ...
  FillChar(Output,
           SizeOf(Output),
           0);
  Output.pSample := MakeSample([0]);
  if Transform.ProcessOutput(0,
                             1,
                             @Output,
                             Status) <> MF_E_BUFFERTOOSMALL then
    raise Exception.Create('Expected MF_E_BUFFERTOOSMALL');
  Output.pSample := nil;

  Provided := MakeSample([0,
                          0]);
  ...
  Check('Caller output',
        Transform.ProcessOutput(0,
                                ...
                                @Output,
                                Status));
  if Output.pSample <> Provided then
    raise Exception.Create('MFT replaced the caller sample');
  CheckBytes('caller output',
             Provided,
             [128, 129]);
  CheckBytes('input unchanged',
             Input,
             [129, 130]);
  Check('Caller output time',
        Provided.GetSampleTime(@SampleTime));
  Check('Caller output duration',
        Provided.GetSampleDuration(@SampleDuration));
  Check('Caller output flags',
        Provided.GetSampleFlags(@SampleFlags));
  if (SampleTime <> 0) or (SampleDuration <> 20000) or (SampleFlags <> 1) or
     (MFGetAttributeUINT32(Provided,
                           MFSampleExtension_CleanPoint,
                           0) <> 1) then
    raise Exception.Create('Caller sample metadata was not copied');
  Output.pSample := nil;

  Check('Drain after caller output',
        Transform.ProcessMessage(MFT_MESSAGE_COMMAND_DRAIN,
                                 0));
  Provided := MakeSample([0]);
  Output.pSample := Provided;
  Check('Caller tail output',
        Transform.ProcessOutput(0,
                                1,
                                @Output,
                                Status));
  CheckBytes('caller tail',
             Provided,
             [130]);
  Check('Caller tail time',
        Provided.GetSampleTime(@SampleTime));
  if (SampleTime <> 20000) then
    raise Exception.Create('Wrong caller tail time');
  Output.pSample := nil;
  ...
  Writeln('PASS: MFT-provided and caller-provided samples, retry, metadata, drain: Wait for shutdown please.');
end;
...
begin
  ...
  try
    ...
    try
      Run();
      Sleep(10000); // Wait before closing when running from the IDE.
    finally
      MFShutdown();
    end;
