...
// Module: RegisteredTopologyPlayer.pas
// Kind: Pascal Unit
// Release date: 13-08-2026
// Language: ENU
//
// Version: 4.0.0
// Description: Discovers the registered Sample 5 grayscale MFT with MFTEnumEx
//              and plays video through a Media Session topology.
//
// Company: FactoryX
...
begin
  ...
  if AHr = MF_E_TOPO_CODEC_NOT_FOUND then
...
function TTopologyPlayer.ActivateRegisteredTransform(
  out ATransform: IMFTransform): HRESULT;
var
...
begin
  ...
  Result := MFTEnumEx(MFT_CATEGORY_VIDEO_EFFECT,
  ...
  try
    for I := 0 to Integer(Count) - 1 do
      if SUCCEEDED(Activates^[I].GetGUID(MFT_TRANSFORM_CLSID_Attribute,
                                        CandidateClsid)) and
         IsEqualGUID(CandidateClsid, CLSID_FactoryXGrayscaleMFT) then
        begin
          Found := True;
...
begin
  ...
  if SUCCEEDED(Result) and AudioFound then
    Result := AudioSourceNode.SetUnknown(MF_TOPONODE_PRESENTATION_DESCRIPTOR,
                                         Presentation);
        ...
        if SUCCEEDED((MediaEvent as IMFAttributes).GetUINT32(
          ...
          begin
            ...
            PostNotice(tekTopologyReady,
              'Topology ready: registered grayscale video plus audio pass-through.');
          end;
      end;
