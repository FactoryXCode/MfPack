// FactoryX
//
// Test harness for TMfGrayscaleMFT. A synthetic RGB32 video frame keeps the
// example focused on the IMFTransform contract instead of file decoding.
...
const
  COLORS: array[0..6] of TColor =
    (clRed, $000080FF, clYellow, clLime, clAqua, clBlue, clFuchsia);
var
  I: Integer;
  LeftEdge: Integer;
  RightEdge: Integer;
begin
  ...
  FSourceBitmap.Canvas.Ellipse(105, 55, 295, 245);
  ...
  FSourceBitmap.Canvas.TextOut(126, 126, 'RGB32 MFT');
...
begin
  ...
  if SUCCEEDED(Result) then
    Result := MFSetAttributeSize(AMediaType, MF_MT_FRAME_SIZE,
  ...
  if SUCCEEDED(Result) then
    Result := MFSetAttributeRatio(AMediaType, MF_MT_FRAME_RATE, 30, 1);
  if SUCCEEDED(Result) then
  ...
  try
    for Y := 0 to ABitmap.Height - 1 do
      begin
        ...
        Move(SourceRow^, DestinationRow^, Stride);
      end;
  finally
    Buffer.Unlock;
  end;
...
begin
  ...
  CheckHr('Create RGB32 media type', CreateVideoType(MediaType));

  CheckHr('Test input type',
    Transform.SetInputType(0, MediaType, MFT_SET_TYPE_TEST_ONLY));
  CheckHr('Set input type', Transform.SetInputType(0, MediaType, 0));
  CheckHr('Test output type',
    Transform.SetOutputType(0, MediaType, MFT_SET_TYPE_TEST_ONLY));
  CheckHr('Set output type', Transform.SetOutputType(0, MediaType, 0));

  CheckHr('Create input sample',
          CreateSampleFromBitmap(FSourceBitmap, InputSample));
  CheckHr('ProcessInput', Transform.ProcessInput(0, InputSample, 0));

  FillChar(OutputData, SizeOf(OutputData), 0);
  OutputStatus := 0;
  Hr := Transform.ProcessOutput(0, 1, @OutputData, OutputStatus);
  CheckHr('ProcessOutput', Hr);
  if not Assigned(OutputData.pSample) then
    raise Exception.Create('ProcessOutput returned no sample.');

  CheckHr('Copy output sample',
          CopySampleToBitmap(OutputData.pSample, FOutputBitmap));
  imgOutput.Picture.Assign(FOutputBitmap);
