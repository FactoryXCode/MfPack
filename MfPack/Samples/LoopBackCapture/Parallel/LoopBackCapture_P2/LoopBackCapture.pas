...
// Description: The audio loopbackcapture engine using parallel tasks.
//
...
type
  ...
  {$endregion}
    ...
    property DeviceStatus: TDeviceState read pvDeviceState;
...
begin
  // The capture thread wil shut down when the status <> rendering.
  // wait for the thread to complete then clean up all the stuff we
    ...
    begin
      ...
      if SUCCEEDED(hr) then
        begin
          ...
          // Skip silence.
          // Only when a sound is detected, the app will act and process data.
...
//  Here we use another approach, using TTask.
