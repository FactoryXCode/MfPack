...
//              This is the basic class of MfPlayer,
...
type
  ...
  {public methods}
    ...
    property Volume: FLOAT read m_fVolume write SetVolume;
...
var
  ...
  nChannels: UINT32;
  fVolume: FLOAT;
  // faVolumes: TFloatArray;  // example, see comment below
...
begin
  ...
  case Self.GetState of
    CmdOpenPending: SetWindowText(m_hwndVideo,
                                  'Opening media... ' +
                                  ExtractFileName(FFileName) + '"');

    CmdReady:       SetWindowText(m_hwndVideo,
                                  'Session is ready.');

    CmdClosing:     SetWindowText(m_hwndVideo,
                                  'Closing session...');
    CmdStart:       begin
   ...
   CmdPause:        SetWindowText(m_hwndVideo,
                                  'Paused.');

   CmdStop:         SetWindowText(m_hwndVideo,
                                  'Stopped.');

   CmdClosed:       SetWindowText(m_hwndVideo,
                                  'Session closed.');
...
//initialization
  // Initializing com and Mf could also be done here
...
//finalization
  // Releasing com and shutdown Mf could also be done here
  // CoUninitialize;
  // MFShutdown;
end.
