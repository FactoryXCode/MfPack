...
type
  PWasapiInputRoute = ^CWasapiInputRoute;
...
begin
  ...
  while WaitForMultipleObjects(2, @WaitHandles[0], False, INFINITE) =
        WAIT_OBJECT_0 + 1 do
  begin
    PacketFrames := 0;
    ...
    begin
      ...
      begin
        ...
        if (FramesToWrite > 0) and
           SUCCEEDED(Route.RenderClient.GetBuffer(FramesToWrite, RenderData)) then
        begin
          if (Flags and AUDCLNT_BUFFERFLAGS_SILENT) <> 0 then
            Route.RenderClient.ReleaseBuffer(FramesToWrite,
                                             AUDCLNT_BUFFERFLAGS_SILENT)
          else
          begin
            CopyMemory(RenderData, CaptureData,
                       FramesToWrite * Route.BlockAlign);
...
function CWasapiInputRoute.Start(
  const HideFromVolumeMixer: Boolean): Boolean;
var
...
begin
  ...
  Hr := CaptureEndpoint.Activate(IID_IAudioClient, CLSCTX_INPROC_SERVER, nil,
                                 Pointer(CaptureAudioClient));
  if FAILED(Hr) then
    Exit;
  Hr := RenderEndpoint.Activate(IID_IAudioClient, CLSCTX_INPROC_SERVER, nil,
                                Pointer(RenderAudioClient));
  if FAILED(Hr) then
    Exit;
  ...
  CaptureFlags := AUDCLNT_STREAMFLAGS_EVENTCALLBACK or
                  AUDCLNT_STREAMFLAGS_NOPERSIST;
  RenderFlags := AUDCLNT_STREAMFLAGS_AUTOCONVERTPCM or
                 AUDCLNT_STREAMFLAGS_SRC_DEFAULT_QUALITY or
                 AUDCLNT_STREAMFLAGS_NOPERSIST;
  if HideFromVolumeMixer then
    RenderFlags := RenderFlags or AUDCLNT_SESSIONFLAGS_DISPLAY_HIDE;

  Hr := CaptureAudioClient.Initialize(AUDCLNT_SHAREMODE_SHARED, CaptureFlags,
                                      500000, 0, MixFormat, @CaptureGuid);
  if SUCCEEDED(Hr) then
    Hr := RenderAudioClient.Initialize(AUDCLNT_SHAREMODE_SHARED, RenderFlags,
                                       500000, 0, MixFormat, @RenderGuid);
  ...
  if (ShutdownEvent = 0) or (SamplesReadyEvent = 0) then
  begin
    Stop;
    Exit;
  end;
  ...
  if SUCCEEDED(Hr) then
    Hr := CaptureAudioClient.GetService(IID_IAudioCaptureClient, CaptureClient);
  if SUCCEEDED(Hr) then
    Hr := RenderAudioClient.GetService(IID_IAudioRenderClient, RenderClient);
  if SUCCEEDED(Hr) then
  ...
  // Prime the speaker endpoint so it starts without an underrun.
  Hr := RenderClient.GetBuffer(RenderBufferFrames, RenderData);
  if SUCCEEDED(Hr) then
    Hr := RenderClient.ReleaseBuffer(RenderBufferFrames,
                                     AUDCLNT_BUFFERFLAGS_SILENT);
  if FAILED(Hr) then
  begin
    Stop;
  ...
  ThreadId := 0;
  WorkerThread := BeginThread(nil, 0, Addr(WasapiRouteThread), Self, 0,
                              ThreadId);
...
begin
  ...
  if Assigned(CaptureAudioClient) then
    CaptureAudioClient.Stop;
  if Assigned(RenderAudioClient) then
    RenderAudioClient.Stop;
  CaptureClient := nil;
  RenderClient := nil;
  CaptureAudioClient := nil;
  RenderAudioClient := nil;
  if SamplesReadyEvent <> 0 then
  begin
