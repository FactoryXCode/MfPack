...
type
  ...
  public
    ...
    function HandlesMessage(_hwnd: HWND;
                            message: UINT): Boolean; override;
    function MessageHandler(_hWnd: HWND;
                            message: TMessage): INT_PTR; override;
  end;
...
//
procedure _MessageHandler(hWaveIn: IntPtr;
                         uMsg: UINT;
                         dwInstance: DWord;
                         wPar: WPARAM;
                         lPar: LPARAM); stdcall;
...
begin
  ...
  mmr := waveInOpen(@_waveHandle,
                    WAVE_MAPPER,
                    @waveFormat,
                    DWORD_PTR(_AppWindow),
                    0,
                    CALLBACK_WINDOW or WAVE_MAPPED_DEFAULT_COMMUNICATION_DEVICE);
...
function CWaveChat.HandlesMessage(_hwnd: HWND;
                                  message: UINT): Boolean;
begin
  case message of
    MM_WIM_OPEN,
    MM_WIM_CLOSE,
...
//
function CWaveChat.MessageHandler(_hWnd: HWND;
                                  message: TMessage): INT_PTR;
var
  mmr: MMRESULT;
  hwaveHandle: HWAVEIN;
  waveHeader: LPWAVEHDR;
