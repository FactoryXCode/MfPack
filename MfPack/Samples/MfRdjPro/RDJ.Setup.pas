...
begin
  ...
  // Artwork
  ASetupRec.LocalArtworkDir := 'Artwork';
  ASetupRec.LocalArtworkPath := Format('%s\%s\', [ExtractFileDir(Application.ExeName),
                                                 ASetupRec.LocalArtworkDir]);

  if not DirectoryExists(ASetupRec.LocalArtworkPath) then
    if not CreateDir(ASetupRec.LocalArtworkPath) then
      ASetupRec.LocalArtworkPath := ExpandFileName(ExtractFileDir(Application.ExeName));
