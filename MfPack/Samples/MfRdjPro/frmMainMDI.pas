...
type
  ...
  private
    ...
    function SetupFoldersExist(out AMissingFolders: string): Boolean;
...
begin
  ...
  if FileExists(FSetupFileName) and
     SetupFoldersExist(MissingFolders) then
    ApplySetupOnce()
  else
    begin
      if (MissingFolders <> '') then
        MessageDlg('One or more folders stored in the setup file are no longer available:' +
                   ...
                   'Please select valid folders in Setup.',
                   mtWarning,
                   [mbOK],
                   0);
...
function TMainMDIFrm.SetupFoldersExist(out AMissingFolders: string): Boolean;
var
  Missing: TStringList;

  procedure CheckFolder(const ASettingName,
                              AFolderName: string);
  var
    FolderName: string;
  ...
  begin
    ...
    try
      ...
      if not DirectoryExists(FolderName) then
        Missing.Add(ASettingName + ': ' + FolderName);
  ...
  procedure CheckFileFolder(const ASettingName,
                                  AFileName: string);
  var
    FolderName: string;
...
begin
  ...
  try
    ...
    CheckFolder('Audio recordings',
                FSetup.AudioRecordingsPath);
    CheckFolder('Video recordings',
                FSetup.VideoRecordingsPath);
    CheckFolder('Database',
                FSetup.DatabasePath);
    CheckFolder('Caddy directory',
                FSetup.CaddyDir);
    CheckFileFolder('Caddy configuration',
                    FSetup.CaddyConfigFile);
    CheckFileFolder('Caddy now-playing JSON',
                    FSetup.CaddyNowPlayingJsonFile);
    CheckFolder('Caddy artwork',
                FSetup.CaddyArtworkPath);
    CheckFolder('Caddy video',
                FSetup.CaddyVideoPath);

    AMissingFolders := Missing.Text;
...
begin
  ...
  JsonDirectory := ExtractFileDir(JsonFile);
  if (JsonDirectory <> '') and
     (not DirectoryExists(JsonDirectory)) then
    Exit;
  ...
  try
    ...
    on E: Exception do
      OutputDebugString(PChar('RDJ Pro could not write now-playing status: ' +
                              E.ClassName + ': ' + E.Message));
  end;
end;
