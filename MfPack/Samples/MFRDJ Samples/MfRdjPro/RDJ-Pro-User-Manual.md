...
RDJ Pro can publish its browser broadcast through FxServe or Caddy. FxServe is
the smaller choice made for the RDJ web application and live fMP4 stream. Caddy
is useful when the server also hosts other websites or needs more proxy options.

The setup page keeps the existing **Caddy / json settings** name for
compatibility. The same fields accept FxServe paths. See
[RDJ Web Server Setup](../../FxServe/RDJ-Web-Server-Setup.md) for the values.
