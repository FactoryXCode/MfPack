...
FxServe and Caddy can both host the RDJ Pro browser application and live fMP4
stream. FxServe is the smaller choice for a dedicated RDJ Pro server. Caddy is
useful when the server also hosts other websites or needs more proxy options.

For FxServe, install the package in `C:\FxServe` and use these RDJ Pro paths:

```text
Root Path:       \\ServerName\FxServe\www
Config Path:     \\ServerName\FxServe\FxServe.ini
json Path:       \\ServerName\FxServe\www\nowplaying.json
Artwork Path:    \\ServerName\FxServe\www\Artwork
Video Path:      \\ServerName\FxServe\www\Stream
```

See [RDJ Web Server Setup](../../../FxServe/RDJ-Web-Server-Setup.md) for the
short installation and router instructions.
