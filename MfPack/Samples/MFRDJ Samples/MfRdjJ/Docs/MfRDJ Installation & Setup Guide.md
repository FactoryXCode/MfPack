...
Version 3.3 - Windows 10/11 - MfRDJ, Icecast, FxServe and Caddy

> **Web-server update:** RDJ can now use FxServe or Caddy. RDJ still sends audio
> to Icecast. FxServe or Caddy hosts the web player, metadata, artwork, and the
> public HTTPS route. See [RDJ Web Server Setup](../../../FxServe/RDJ-Web-Server-Setup.md)
> for the current paths and short installation steps. The detailed Caddy
> examples below remain available for Caddy users.
...
| Direct public Icecast port | Do not forward `8000` unless direct Icecast access is required |
...
3. At minimum, set these XML values:
   ...
   - `<listen-socket><port>` to `8000`, unless you use another port;
