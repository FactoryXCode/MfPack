...
RDJ still sends its audio stream to Icecast. FxServe or Caddy provides the web
page, artwork, `nowplaying.json`, public HTTPS, and the secure route to Icecast.
FxServe is the smaller choice for a dedicated RDJ server. Caddy is useful for a
server that also handles other websites or more complex proxy rules.

Some setup fields still use **Caddy** in their names. These fields are also used
for FxServe paths. See [RDJ Web Server Setup](../../../FxServe/RDJ-Web-Server-Setup.md)
