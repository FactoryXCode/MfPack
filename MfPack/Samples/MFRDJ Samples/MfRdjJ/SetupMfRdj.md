...
## Web server: FxServe or Caddy

FxServe and Caddy can both host the RDJ web page and provide public HTTPS.
FxServe is the smaller choice for a dedicated RDJ server. Caddy is useful when
the server also hosts other websites or needs more proxy options.

For FxServe, install the package in `C:\FxServe` and use these RDJ paths:

```text
Web root:       \\ServerName\FxServe\www
Now playing:    \\ServerName\FxServe\www\nowplaying.json
Artwork:        \\ServerName\FxServe\www\Artwork
```

RDJ still uses Icecast for its audio stream. FxServe proxies the public `/live`
route to Icecast in the same way as the supplied Caddy setup.

See [RDJ Web Server Setup](../../FxServe/RDJ-Web-Server-Setup.md) for the short
installation and router instructions.
