...
// Module: MfSubtitleTransform.pas
// Kind: Pascal Unit
...
type
  ...
  private
    ...
    FTimeline: TMfMediaTimeline;
    FFrameIndex: UINT64;
    FLoggedTimelineFallback: Boolean;
    FLoggedFirstComposite: Boolean;
...
var
  ...
  requiredSize: UInt64;
  mediaTimeMs: Int64;
  timeStatus: TMfSampleTimeStatus;
...
begin
  ...
  try
    if Assigned(FTimeline) then
      begin
        mediaTimeMs := FTimeline.ResolveSampleTimeMs(Sample,
                                                     FFrameIndex,
                                                     timeStatus);
        if (timeStatus <> stsFromSample) and
           (not FLoggedTimelineFallback) then
          begin
            OutputDebugString(PChar(Format('MfPlayer X2 subtitle transform uses timeline fallback status=%d',
                                           [Ord(timeStatus)])));
            FLoggedTimelineFallback := True;
          end;
      end
    else
      begin
        if FAILED(Sample.GetSampleTime(@sampleTime)) then
          sampleTime := 0;
        mediaTimeMs := sampleTime div 10000;
      end;

    Inc(FFrameIndex);

    requiredSize := UInt64(FStride) * UInt64(FHeight);
    bufferSize := currentLength;
    if (UInt64(bufferSize) < requiredSize) and
       (UInt64(maxLength) >= requiredSize) then
      bufferSize := maxLength;

    if not FLoggedFirstComposite then
      begin
        OutputDebugString(PChar(Format('MfPlayer X2 subtitle first frame time=%d size=%d max=%d stride=%d %dx%d',
                                       [mediaTimeMs, bufferSize, maxLength, FStride, FWidth, FHeight])));
        FLoggedFirstComposite := True;
      end;
