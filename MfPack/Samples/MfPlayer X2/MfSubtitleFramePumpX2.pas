...
// Module: MfSubtitleFramePumpX2.pas
// Kind: Pascal Unit
...
begin
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: cancel helper started'));
  ...
  try
    if Assigned(FReader) then
      begin
        OutputDebugString(PChar('MfPlayer X2 Export: cancel helper before reader Flush'));
        hr := FReader.Flush(MF_SOURCE_READER_FIRST_VIDEO_STREAM);
        OutputDebugString(PChar(Format('MfPlayer X2 Export: cancel helper after reader Flush hr=%.8x', [DWORD(hr)])));
      end;
    ...
    if comInitialized then
      CoUninitialize();
    OutputDebugString(PChar('MfPlayer X2 Export: cancel helper done'));
  end;
end;
...
begin
  ...
  OutputDebugString(PChar(Format('MfPlayer X2 Export: audio input channels=%d rate=%d bits=%d aacBytesPerSec=%d',
  ...
  if FAILED(Result) then
    begin
      ...
      OutputDebugString(PChar(Format('MfPlayer X2 Export: audio AddStream failed hr=%.8x',
                                     [DWORD(Result)])));
      Exit;
    end;
  ...
  Result := Writer.SetInputMediaType(AudioStreamIndex,
                                     ...
                                     nil);
  if FAILED(Result) then
    OutputDebugString(PChar(Format('MfPlayer X2 Export: audio SetInputMediaType failed hr=%.8x', [DWORD(Result)])));
end;
...
begin
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: cancel requested'));
...
begin
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: pause requested'));
...
begin
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: resume requested'));
  FPauseRequested := False;
end;
...
begin
  ...
  OutputDebugString(PChar(Format('MfPlayer X2 Export: paused at frames=%d time=%d',
  ...
  OutputDebugString(PChar(Format('MfPlayer X2 Export: resumed after %d ms',
                                 [pauseElapsedMs])));
  Result := FCancelRequested;
end;
...
begin
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: creating source reader'));
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: configuring source reader'));
  ...
  OutputDebugString(PChar('MfPlayer X2 Export: creating sink writer'));
  ...
  if FAILED(audioHr) then
    begin
      ...
      OutputDebugString(PChar(Format('MfPlayer X2 Export: audio reader disabled hr=%.8x', [DWORD(audioHr)])));
      hasAudio := False;
      audioReader := nil;
      audioType := nil;
    end;
    ...
    begin
      ...
      if FAILED(audioHr) then
        begin
          ...
          OutputDebugString(PChar(Format('MfPlayer X2 Export: audio stream disabled hr=%.8x', [DWORD(audioHr)])));
          hasAudio := False;
          audioReader := nil;
          audioType := nil;
        end
        ...
        begin
          ...
          OutputDebugString(PChar(Format('MfPlayer X2 Export: audio stream enabled index=%d bytesPerSec=%d', [audioStreamIndex, audioState.BytesPerSecond])));
        end;
    end;
    ...
    begin
      ...
      if (FFramesWritten <> lastReadDebugFrame) and ((FFramesWritten mod 250) = 0) then
        begin
          ...
          OutputDebugString(PChar(Format('MfPlayer X2 Export: before ReadSample frames=%d time=%d', [FFramesWritten, outputTime])));
        end;
      ...
      if (FFramesWritten <> lastWriteDebugFrame) and ((FFramesWritten mod 250) = 0) then
        begin
          ...
          OutputDebugString(PChar(Format('MfPlayer X2 Export: before WriteSample frames=%d time=%d', [FFramesWritten, outputTime])));
        end;
      ...
      repeat
        ...
        if (writeRetryCount = 1) or ((writeRetryCount mod 20) = 0) then
          OutputDebugString(PChar(Format('MfPlayer X2 Export: video WriteSample waiting for encoder frames=%d retries=%d', [FFramesWritten, writeRetryCount])));
        Sleep(25);
  ...
  if Assigned(writer) and
    ...
    begin
      ...
      OutputDebugString(PChar(Format('MfPlayer X2 Export: before audio copy stop=%d',
      ...
      audioHr := WriteAudioSamples(audioReader,
                                   ...
                                   audioState,
                                   lastOutputTime + frameDuration);
      OutputDebugString(PChar(Format('MfPlayer X2 Export: after audio copy hr=%.8x',
                                     [DWORD(audioHr)])));
    end;
  ...
  if Assigned(writer) and SUCCEEDED(Result) and (FFramesWritten > 0) then
    begin
      ...
      OutputDebugString(PChar(Format('MfPlayer X2 Export: before Finalize cancel=%d',
                                     [Integer(FCancelRequested)])));
      Result := writer.Finalize();
      OutputDebugString(PChar(Format('MfPlayer X2 Export: after Finalize hr=%.8x',
                                     [DWORD(Result)])));
    end
  else
    OutputDebugString(PChar(Format('MfPlayer X2 Export: skipping Finalize hr=%.8x cancel=%d frames=%d', [DWORD(Result), Integer(FCancelRequested), FFramesWritten])));
  ...
  FWriter := nil;
  FReader := nil;
  OutputDebugString(PChar(Format('MfPlayer X2 Export: done hr=%.8x frames=%d videoSec=%.3f audioSec=%.3f audioBps=%d', [DWORD(Result), FFramesWritten, (lastOutputTime + frameDuration) / 10000000.0, audioState.OutputTime / 10000000.0, audioState.BytesPerSecond])));
end;
