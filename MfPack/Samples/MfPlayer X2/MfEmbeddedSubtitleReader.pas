...
class function TMfEmbeddedSubtitleReader.FormatName(
  const SubtitleFormat: TMfEmbeddedSubtitleFormat): string;
begin
...
begin
  ...
  Text := StringReplace(Text, '\N', #10, [rfReplaceAll]);
  Text := StringReplace(Text, '\n', #10, [rfReplaceAll]);
  Text := StringReplace(Text, '\h', ' ', [rfReplaceAll]);
...
begin
  ...
  // Creating a second Media Foundation source for a large MKV can be much
  // slower than reading its compact EBML metadata. Prefer the direct parser
  // and use Source Reader enumeration only as a compatibility fallback.
  if IsMatroska then
    begin
