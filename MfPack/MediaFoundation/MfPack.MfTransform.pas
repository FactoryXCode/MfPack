...
{$WARN BOUNDS_ERROR OFF}
  ...
  _MFT_INPUT_DATA_BUFFER_FLAGS = (
    ...
    {$EXTERNALSYM MFT_INPUT_DATA_BUFFER_PLACEHOLDER}
  );
...
const
  MFT_OUTPUT_DATA_BUFFER_INCOMPLETE	= DWord($1000000);
  {$EXTERNALSYM MFT_OUTPUT_DATA_BUFFER_INCOMPLETE}
  MFT_OUTPUT_DATA_BUFFER_FORMAT_CHANGE	= DWord($100);
  {$EXTERNALSYM MFT_OUTPUT_DATA_BUFFER_FORMAT_CHANGE}
  MFT_OUTPUT_DATA_BUFFER_STREAM_END	= DWord($200);
  {$EXTERNALSYM MFT_OUTPUT_DATA_BUFFER_STREAM_END}
  MFT_OUTPUT_DATA_BUFFER_NO_SAMPLE	= DWord($300);
...
const
  MFT_INPUT_STATUS_ACCEPT_DATA = DWord($1);
...
const
  MFT_OUTPUT_STATUS_SAMPLE_READY	= DWord($1);
...
const
  MFT_INPUT_STREAM_WHOLE_SAMPLES	          = DWord($1);
  {$EXTERNALSYM MFT_INPUT_STREAM_WHOLE_SAMPLES}
  MFT_INPUT_STREAM_SINGLE_SAMPLE_PER_BUFFER	= DWord($2);
  {$EXTERNALSYM MFT_INPUT_STREAM_SINGLE_SAMPLE_PER_BUFFER}
  MFT_INPUT_STREAM_FIXED_SAMPLE_SIZE	      = DWord($4);
  {$EXTERNALSYM MFT_INPUT_STREAM_FIXED_SAMPLE_SIZE}
  MFT_INPUT_STREAM_HOLDS_BUFFERS	          = DWord($8);
  {$EXTERNALSYM MFT_INPUT_STREAM_HOLDS_BUFFERS}
  MFT_INPUT_STREAM_DOES_NOT_ADDREF	        = DWord($100);
  {$EXTERNALSYM MFT_INPUT_STREAM_DOES_NOT_ADDREF}
  MFT_INPUT_STREAM_REMOVABLE	              = DWord($200);
  {$EXTERNALSYM MFT_INPUT_STREAM_REMOVABLE}
  MFT_INPUT_STREAM_OPTIONAL	                = DWord($400);
  {$EXTERNALSYM MFT_INPUT_STREAM_OPTIONAL}
  MFT_INPUT_STREAM_PROCESSES_IN_PLACE	      = DWord($800);
...
const
  ...
  //
  MFT_OUTPUT_STREAM_WHOLE_SAMPLES	            = DWord($1);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_WHOLE_SAMPLES}
  MFT_OUTPUT_STREAM_SINGLE_SAMPLE_PER_BUFFER	= DWord($2);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_SINGLE_SAMPLE_PER_BUFFER}
  MFT_OUTPUT_STREAM_FIXED_SAMPLE_SIZE	        = DWord($4);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_FIXED_SAMPLE_SIZE}
  MFT_OUTPUT_STREAM_DISCARDABLE	              = DWord($8);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_DISCARDABLE}
  MFT_OUTPUT_STREAM_OPTIONAL	                = DWord($10);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_OPTIONAL}
  MFT_OUTPUT_STREAM_PROVIDES_SAMPLES	        = DWord($100);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_PROVIDES_SAMPLES}
  MFT_OUTPUT_STREAM_CAN_PROVIDE_SAMPLES	      = DWord($200);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_CAN_PROVIDE_SAMPLES}
  MFT_OUTPUT_STREAM_LAZY_READ	                = DWord($400);
  {$EXTERNALSYM MFT_OUTPUT_STREAM_LAZY_READ}
  MFT_OUTPUT_STREAM_REMOVABLE	                = DWord($800);
...
const
  ...
  // Carried over from DMO (IMediaObject)
  //
  MFT_SET_TYPE_TEST_ONLY	= DWord($00000001); // check but don't set

  //
  // not carried over from DMO - use Nil type to unset.
  //
  MFT_SET_TYPE_CLEAR      = Dword($00000002); // unset
...
const
    MFT_PROCESS_OUTPUT_DISCARD_WHEN_NO_BUFFER	= DWord($1); // discard this sample if pSample ptr is Nil.
    MFT_PROCESS_OUTPUT_REGENERATE_LAST_OUTPUT = DWord($2); // New flags for MFTs
...
const
  MFT_PROCESS_OUTPUT_STATUS_NEW_STREAMS	= DWord($100);  // Output flag
...
const
  MF3DVideoOutputType_BaseView = UINT32(0);
  MF3DVideoOutputType_Stereo   = UINT32(1);
