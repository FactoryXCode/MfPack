...
const
  MFASF_SPLITTER_REVERSE = DWord($1);
  {$EXTERNALSYM MFASF_SPLITTER_REVERSE}
  MFASF_SPLITTER_WMDRM   = DWord($2);
...
const
  ASF_STATUSFLAGS_INCOMPLETE     = DWord($1);
  {$EXTERNALSYM ASF_STATUSFLAGS_INCOMPLETE}
  ASF_STATUSFLAGS_NONFATAL_ERROR = DWord($2);
...
const
  MFASF_MULTIPLEXER_AUTOADJUST_BITRATE = DWord($1);
...
const
  MFASF_INDEXER_WRITE_NEW_INDEX          = DWord($1);
  {$EXTERNALSYM MFASF_INDEXER_WRITE_NEW_INDEX}
  MFASF_INDEXER_READ_FOR_REVERSEPLAYBACK = DWord($2);
  {$EXTERNALSYM MFASF_INDEXER_READ_FOR_REVERSEPLAYBACK}
  MFASF_INDEXER_WRITE_FOR_LIVEREAD       = DWord($4);
...
const
  MFASF_STREAMSELECTOR_DISABLE_THINNING    = DWord($1);
  {$EXTERNALSYM MFASF_STREAMSELECTOR_DISABLE_THINNING}
  MFASF_STREAMSELECTOR_USE_AVERAGE_BITRATE = DWord($2);
...
const
  ASF_STATUS_NOTSELECTED     = DWord(0);
  {$EXTERNALSYM ASF_STATUS_NOTSELECTED}
  ASF_STATUS_CLEANPOINTSONLY = DWord(1);
  {$EXTERNALSYM ASF_STATUS_CLEANPOINTSONLY}
  ASF_STATUS_ALLDATAUNITS    = DWord(2);
...
type
  ...
  _MFSINK_WMDRMACTION             = (
    ...
    {$EXTERNALSYM MFSINK_WMDRMACTION_UNDEFINED}
    ...
    {$EXTERNALSYM MFSINK_WMDRMACTION_ENCODE}
    ...
    {$EXTERNALSYM MFSINK_WMDRMACTION_TRANSCODE}
    ...
    {$EXTERNALSYM MFSINK_WMDRMACTION_TRANSCRYPT}
    MFSINK_WMDRMACTION_LAST       = 3   // Reserved. Do not use.
    {$EXTERNALSYM MFSINK_WMDRMACTION_LAST}
  );
...
type
  ...
  ['{53590F48-DC3B-4297-813F-787761AD7B3E}']
    ...
    function GetSeekPositionForValue(pvarValue: MfPROPVARIANT;
