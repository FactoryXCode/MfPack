...
const
  ...
  SDK_API_VERSION = '10.0.22621.0';  // Latest API version.

type

  TReferenceArray = array[0..9] of string;
  ...
  TInformationStyle = (isCode,  // Process the result in your code, using the property values.
                       isDialog, // Process the result by the FactoryX HResult Tool Dialog.
                       isIDEDebugWindow, // Process the code within the IDE message window.
                       isDOS); // Process the code in the commandline window.
  ...
  public
    ...
    property SelectedHandling: TInformationStyle read InformationStyle write InformationStyle;
...
label
  done_HLD,         // Use the HResult Lookup Dialog - this application lookup tools-. (recommended)
  done_MSELTDESC,   // Same as done_MSELTDOS, but hide DOSbox and print the results in the HResult Lookup dialog.
  done_SYSEMLTDLG;  // Use SysErrorMessage API methods. (Basic lookup)
...
done_HLD:
  ...
  else if (InformationStyle = isDialog) then
    begin
