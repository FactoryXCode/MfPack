...
Default YourRemoteServeName profile:
  
```text
Server:                   Your remote server name, like: YourRemoteServeName  
Service:                  The service name like: FxServe  
FxServe share:       Your remote server path, like: \\YourRemoteServeName\FxServe  
Server local path:   Your remote server local path, like: C:\FxServe  
Public hostname:   Your public hostname, like: yourradio.yourhost.com  
```
...
- Show a visible processing status, hourglass cursor, and disabled command
  controls while remote status, start, stop, restart, install, uninstall,
  health-check, or configuration-apply operations are in progress.
...
- **Save & apply** also writes the public hostname to `[Wan] HostName` in the
  deployed `FxServe.ini` and writes `www\fxserve-config.json` with the selected
  server name, LAN URL, public hostname, and public URL. Existing files receive
  a `.bak` backup before replacement.

Runtime publisher files such as `live.json`, `viewers.json`,
`broadcast_status.json`, `nowplaying.json`, media fragments, and artwork are not
configuration files and are deliberately left unchanged. Browser and PWA files
use relative same-origin URLs, so they continue to work through both the LAN
server name and public hostname without hard-coded addresses.
