...
{$APPTYPE CONSOLE}
{$R 'Build\SelfContained\FxServe.Bootstrap.res'}
...
begin
  ...
  Writeln('FxServe - FactoryX HTTP media server');
  Writeln('');
  Writeln('Usage:');
  Writeln('  FxServe.exe [options]');
  Writeln('');
  Writeln('Run options:');
  Writeln('  --config <file>, -c <file>');
  Writeln('      Use the specified configuration file.');
  Writeln('  --install');
  Writeln('      Install FxServe as an automatic Windows service.');
  Writeln('  --uninstall');
  Writeln('      Remove the FxServe Windows service.');
  Writeln('  --service');
  Writeln('      Run as a Windows service. Used internally by Service Control Manager.');
  Writeln('  --bootstrap');
  Writeln('      Create any missing configuration and web files, then exit.');
  Writeln('  --bootstrap-refresh');
  Writeln('      Refresh embedded static web files, preserving site and live configuration.');
  Writeln('');
  Writeln('Certificate options:');
  Writeln('  --certificate-setup --host <name> --email <address>');
  Writeln('      --accept-ca-terms [--staging]');
  Writeln('      Configure built-in ACME certificate management.');
  Writeln('  --certificate-disable');
  Writeln('      Disable built-in certificate management.');
  Writeln('');
  Writeln('Help:');
  Writeln('  --help, -h, /?');
  Writeln('      Show this information and exit.');
  Writeln('');
  Writeln('With no mode option, FxServe runs interactively.');
end;
...
begin
  ...
  if HasSwitch('--help') or HasSwitch('-h') or HasSwitch('/?') then
    begin
      ShowHelp();
      Exit;
    end;
  ...
  EnsureFxServeBootstrap(ConfigFileName,
                         HasSwitch('--bootstrap-refresh'));

  if HasSwitch('--bootstrap') or HasSwitch('--bootstrap-refresh') then
    begin
      Writeln('FxServe bootstrap files are ready.');
      Exit;
    end;
