...
function RequestPath(const ATarget: string): string;
var
  QueryPos: Integer;
begin
  QueryPos := Pos('?', ATarget);
  if QueryPos > 0 then
    Result := Copy(ATarget, 1, QueryPos - 1)
  else
    Result := ATarget;
end;
...
begin
  ...
  Error := FxServe.HttpApi.HttpInitialize(Version,
  ...
  if (Error <> NO_ERROR) then
    begin
      ...
      raise Exception.Create('HTTP.sys initialization failed: ' + HttpApiErrorText(Error));
    end;
