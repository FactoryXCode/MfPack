...
- Caddy is already installed and working.
...
Caddy has more features, but it is larger than needed for a basic RDJ stream.
...
3. Open **FxServeAdmin** and enter:
   ...
   ```text
   Server:              YourRemoteServeName
   Service:             FxServe
   FxServe share:       \\YourRemoteServeName\FxServe
   Server local path:   C:\FxServe
   Public hostname:     yourradio.yourhost.com
   ```
...
7. Test the LAN address:
   ...
   ```text
   http://YourRemoteServeName:8080/
   ```

8. Test the public address:

   ```text
   https://yourradio.yourhost.com/
   ```
...
2. Open **CaddyAdmin** and enter:
   ...
   Server:              YourRemoteServeName
   Service:             RDJProCaddy
   Caddy share:         \\YourRemoteServeName\Caddy
...
5. In RDJ Pro Setup, set the streaming output folder to:
   ...
   ```text
   \\YourRemoteServeName\Caddy\stream
   ```
...
```text
Service:             RDJCaddy
Caddy share:         \\YourRemoteServeName\Caddy_RDJ
Server local path:   C:\Caddy_RDJ
Streaming folder:    \\YourRemoteServeName\Caddy_RDJ\stream
```
...
| --- | --- | ---: | ---: | --- |
| FactoryX Radio HTTP | TCP | 80 | 80 | Server IP address |
| FactoryX Radio HTTPS | TCP | 443 | 443 | Server IP address |
