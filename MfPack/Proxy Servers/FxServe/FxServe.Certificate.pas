...
//          Persistent settings are protected below HKLM. Private keys are
...
type
  ...
  IFxServeAcmeChallengeSource = interface ['{D81C50D3-A645-4E90-BE7C-D2154ED558F8}']
...
begin
  ...
  if not ConvertStringSecurityDescriptorToSecurityDescriptorW('D:P(A;;KA;;;SY)(A;;KA;;;BA)',
                                                              SDDL_REVISION_1,
                                                              SecurityDescriptor,
                                                               nil) then
    RaiseLastOSError();
...
const
  ...
  winhttpLib = 'winhttp.dll';

{$WARN SYMBOL_PLATFORM OFF}
  function CryptAcquireContextW; external advapi32Lib name 'CryptAcquireContextW' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptReleaseContext;  external advapi32Lib name 'CryptReleaseContext' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptGetUserKey;      external advapi32Lib name 'CryptGetUserKey' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptGenKey;          external advapi32Lib name 'CryptGenKey' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptDestroyKey;      external advapi32Lib name 'CryptDestroyKey' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptExportKey;       external advapi32Lib name 'CryptExportKey' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptCreateHash;      external advapi32Lib name 'CryptCreateHash' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptHashData;        external advapi32Lib name 'CryptHashData' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptGetHashParam;    external advapi32Lib name 'CryptGetHashParam' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptSignHashW;       external advapi32Lib name 'CryptSignHashW' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptDestroyHash;     external advapi32Lib name 'CryptDestroyHash' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function ConvertStringSecurityDescriptorToSecurityDescriptorW; external advapi32Lib name 'ConvertStringSecurityDescriptorToSecurityDescriptorW' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};

  function CertOpenStore;  external crypt32Lib name 'CertOpenStore' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertCloseStore; external crypt32Lib name 'CertCloseStore' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertEnumCertificatesInStore;       external crypt32Lib name 'CertEnumCertificatesInStore' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertCreateCertificateContext;      external crypt32Lib name 'CertCreateCertificateContext' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertFreeCertificateContext;        external crypt32Lib name 'CertFreeCertificateContext' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertAddCertificateContextToStore;  external crypt32Lib name 'CertAddCertificateContextToStore' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertSetCertificateContextProperty; external crypt32Lib name 'CertSetCertificateContextProperty' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertGetCertificateContextProperty; external crypt32Lib name 'CertGetCertificateContextProperty' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CertGetNameStringW;                external crypt32Lib name 'CertGetNameStringW' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function CryptAcquireCertificatePrivateKey; external crypt32Lib name 'CryptAcquireCertificatePrivateKey' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function NCryptFreeObject;                  external crypt32Lib name 'NCryptFreeObject' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  ...
  function HttpSetServiceConfiguration;    external httpapiLib name 'HttpSetServiceConfiguration' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function HttpDeleteServiceConfiguration; external httpapiLib name 'HttpDeleteServiceConfiguration' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function HttpQueryServiceConfiguration;  external httpapiLib name 'HttpQueryServiceConfiguration' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};

  function WinHttpOpen;            external winhttpLib name 'WinHttpOpen' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpConnect;         external winhttpLib name 'WinHttpConnect' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpOpenRequest;     external winhttpLib name 'WinHttpOpenRequest' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpSendRequest;     external winhttpLib name 'WinHttpSendRequest' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpReceiveResponse; external winhttpLib name 'WinHttpReceiveResponse' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpQueryHeaders;    external winhttpLib name 'WinHttpQueryHeaders' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpReadData;        external winhttpLib name 'WinHttpReadData' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpSetTimeouts;     external winhttpLib name 'WinHttpSetTimeouts' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
  function WinHttpCloseHandle;     external winhttpLib name 'WinHttpCloseHandle' {$IF COMPILERVERSION > 20.0} delayed {$ENDIF};
