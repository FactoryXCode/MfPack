...
type
  ...
  TMfCastTrackKind = (ctkUnknown,
                      ctkVideo,
                      ctkAudio,
                      ctkSubtitle);

  TMfCastTrackSource = (ctsUnknown,
                        ctsMediaFoundation,
                        ctsMatroska,
                        ctsSidecar);
  ...
  TMfCastSubtitleChoiceSource = (cscsSidecar,
                                 cscsEmbedded);

  TMfCastSubtitleChoice = record
    Source: TMfCastSubtitleChoiceSource;
    Language: string;
    DisplayName: string;
    SourceName: string;
    TrackId: Int64;
  ...
  TMfCastSubtitleChoiceArray = array of TMfCastSubtitleChoice;
  ...
  TMfCastTranscodeRequest = record
    ...
    SubtitleStreamIndex: DWORD;
    HasSubtitleStreamIndex: Boolean;
...
procedure TMfCastSubtitleChoice.Reset();
begin
...
begin
  ...
  Result := (Int64(Ord(AKind) + 1) shl 56) or
            (Int64(Ord(ASource) + 1) shl 48) or
            Int64(AStreamIndex);
end;
...
begin
  ...
  Result := (KindValue >= Ord(Low(TMfCastTrackKind))) and
            ...
            (SourceValue <= Ord(High(TMfCastTrackSource)));
  if not Result then
    Exit;

  AKind := TMfCastTrackKind(KindValue);
  ASource := TMfCastTrackSource(SourceValue);
  AStreamIndex := DWORD(ATrackId and $FFFFFFFF);
end;
...
begin
  ...
  Result := 2166136261;
  Text := LowerCase(ExpandFileName(AFileName));
...
begin
  ...
  case AMode of
    csmAutomatic:         Result := 'Automatic';
    csmNone:              Result := 'None';
    csmExternalTextTrack: Result := 'ExternalTextTrack';
    csmBurnIntoVideo:     Result := 'BurnIntoVideo';
  else
    Result := 'Unknown';
  end;
end;
