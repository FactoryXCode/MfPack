...
The other units contain device discovery, Cast V2 TLS/channel handling,   
controller logic, media planning, and the local range-capable HTTP server.  
...
This keeps window-thread plumbing out of application forms without making it  
mandatory for non-VCL or non-windowed API consumers.   
...
These routes control only MfCast playback and cannot override a  
Chromecast/Google TV system mute set by its physical remote.  
...
For an active transcode, **SelectAudioTrack** restarts at the current source position using the  
