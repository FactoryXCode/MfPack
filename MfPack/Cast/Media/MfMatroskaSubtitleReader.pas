...
begin
  ...
  FileSize := (Int64(FileData.nFileSizeHigh) shl 32) or Int64(FileData.nFileSizeLow);
...
begin
  ...
  Result := TLanguageTags.NormalizeLanguageTag(Language);
end;
      ...
      if (Header.ElementId = EBML_ID_TRACK_ENTRY) then
        begin
          ...
          OutputDebugString(PChar(Format(
            'Direct MKV track entry=%d subtitle=%s codec=%s lang=%s name=%s supported=%s',
            [Track.TrackNumber,
             BoolToStr(IsSubtitle, True),
             Track.CodecId,
             Track.Language,
             Track.Name,
             BoolToStr(Track.Supported, True)])));
